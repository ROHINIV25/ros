...
void loop() {
  ...
  bno.getEvent(&orientationData, Adafruit_BNO055::VECTOR_EULER);
    ...
    float theta_error=speed.theta-orientationData.orientation.z;

    float target_w1 = 0.0;
    if (theta_error>=1.0){
        target_w1 = theta_error*10.0;
    }
    else{
        target_w1=0.0;
    } 
...
void loop() {
  ...
  analogWrite(MOTOR2_LPWM_PIN, 150);
  analogWrite(MOTOR2_RPWM_PIN, 0);


  digitalWrite(MOTOR3_DIR_PIN, HIGH);
  analogWrite(MOTOR3_PWM_PIN, 150);


  digitalWrite(MOTOR4_DIR_PIN, HIGH);
  analogWrite(MOTOR4_PWM_PIN, 150);
